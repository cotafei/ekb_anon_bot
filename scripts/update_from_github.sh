...
# Версия: 1.1 (с поддержкой миграции БД)
# ============================================
...
# 2. Создаем резервную копию текущей версии
...
# 3. Клонируем свежую версию с GitHub
...
# 4. СОХРАНЯЕМ ТВОЙ СКРИПТ МИГРАЦИИ!
echo -e "${YELLOW}💾 Сохраняю твой скрипт миграции...${NC}"
if [ -f "src/migrate_db.py" ]; then
    cp src/migrate_db.py /tmp/migrate_db.py.bak
    echo -e "${GREEN}✅ Скрипт миграции сохранен${NC}"
else
    echo -e "${YELLOW}⚠️ Скрипт миграции не найден${NC}"
fi

# 5. Обновляем файлы проекта
...
# ВОССТАНАВЛИВАЕМ ТВОЙ СКРИПТ МИГРАЦИИ
...
# 6. Заменяем старые папки новыми
...
# 8. ЗАПУСКАЕМ МИГРАЦИЮ БД!
echo -e "${YELLOW}🔄 Запускаю миграцию базы данных...${NC}"
cd src && python migrate_db.py
cd ..
...
# 10. Убираем за собой временную папку
